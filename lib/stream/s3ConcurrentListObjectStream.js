 ...
 * @param {Number} [options.maxdepth] Default to -1. How far down the rabbit hole
 *   do we go
 */
function S3ConcurrentListObjectStream (options) {
  ...
  this.queue = async.queue(
    this.listDirectoryAndRecurse.bind(this),
 ...
 */
S3ConcurrentListObjectStream.prototype.listDirectoryAndRecurse = function (
  options,
  callback
) {
  var self = this;
   ...
   */
  function listRecursively (continuationToken) {
    var currentdepth = options.currentdepth || 1
    ...
    self.listDirectoryPage(
      ...
      function (error, nextContinuationToken, s3Objects, commonPrefixes) {
        ...
        // only add to queue if we haven't reached maxdepth yet
        ...
        if (nextContinuationToken) {
          listRecursively(nextContinuationToken);
        }
        else {
          callback();
        }
